...
class ProvinceCodes extends React.Component {
    constructor() {
        super();
    }
    ...
    render() {
        ...
        const {provinceList,cityList,countyList,province,city,area} = this.props;
        let catalogFirstOption = [];
        let catalogSecondOption = [];
        let catalogThirdOption = [];
        if (provinceList && provinceList.length > 0) {
            catalogFirstOption = provinceList.map(function (data) {
                return (
        ...
        if(!city||!area||!city==""||!area==""){
            content = <div>
                <Col span="24">
                    <Select
                        {...getFieldProps(province, {initialValue: '', trigger: 'onSelect'})}
                        size="large"
                        showSearch
                        placeholder="请选第一级"
...
ProvinceCodes.propTypes = propTypes;
 ...
 */
export default connect(stateToProps, dispatchToProps)(ProvinceCodes);
